                       ...
                       struct ovsdb_idl *ovnsb_idl)
{
    ...
    SBREC_RBAC_PERMISSION_FOR_EACH_SAFE (perm_row, ovnsb_idl) {
        if (strcmp(perm_row->table, pcfg->table) == 0) {
            if (ovn_rbac_match_perm(perm_row, pcfg)) {
                pcfg->row = perm_row;
...
/*
 * Make sure that DB Role 'ovn-controller' exists,
 * has no duplicates and its permission list exactly
 * match to predefined permissions.
 * Recreate if matching fails.
 */
static void
                      ...
                      struct ovsdb_idl *ovnsb_idl)
{
    ...
    /*
     * Make sure predefined permissions are presented in the
     * RBAC_Permissions table.
     * Otherwise create consistent permissions.
     */
    ...
    /*
     * Make sure the role 'ovn-controller' is presented in the
     * RBAC_Role table.
     * Otherwise create the role.
     * Remove duplicates if any.
     */
    SBREC_RBAC_ROLE_FOR_EACH_SAFE (role_row, ovnsb_idl) {
        if (strcmp(role_row->name, "ovn-controller") == 0) {
            if (rbac_role) {
